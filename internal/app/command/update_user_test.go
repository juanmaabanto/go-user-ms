...
func Test_Handle_UpdateUser_Find_Error(t *testing.T) {
	// Arrange
...
func Test_Handler_UpdateUser_Not_Found(t *testing.T) {
	// Arrange
	mockRepo := new(mocks.MockRepository[model.User])
	repos := repository.Repositories{
		User: mockRepo,
...
func Test_Handle_UpdateUser_Completed(t *testing.T) {
	// Arrange
	mockRepo := new(mocks.MockRepository[model.User])
	repos := repository.Repositories{
		User: mockRepo,
	...
	item := UpdateUser{Id: expectedId, UserName: "test"}

	mockRepo.On("FindById", ctx, mock.AnythingOfType("string")).Return(user, nil)
	mockRepo.On("UpdateOne", ctx, item.Id, mock.AnythingOfType("model.User")).Return(nil)
